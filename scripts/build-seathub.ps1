...
if (-not $SkipBuild) {
    ...
    try {
        ...
        # The top-level qmake above writes only the top-level Makefile. `moonlight-qt.pro` is
        # TEMPLATE = subdirs, and on a subdirs project the per-subdirectory Makefiles are
        # generated by make, not by that qmake call - the generated `sub-app-release` rule
        # opens with `@if not exist app\ mkdir app\`. So build-x64-release\app does not exist
        # yet on a clean tree and the forced app qmake below cannot cd into it. This is only
        # invisible locally because line 120 reuses the build folder and never cleans it, so
        # app\ survives from an earlier run. CI starts cold every time, which is why this
        # script failed there on its first execution and had never failed here.
        New-Item -ItemType Directory -Force -Path (Join-Path $BuildFolder 'app') | Out-Null
